...
async def ban_user(interaction: discord.Interaction, user: discord.User):
    ...
    if user_id in data['users']:
        ...
        # notify admins
        admin_embed = discord.Embed(title="User Banned", description=f"Admin <@{interaction.user.id}> banned {user.name} ({user.id}) and removed their subdomains.")
        await send_admin_embed(interaction.client, admin_embed)
    else:
